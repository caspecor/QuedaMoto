...
export function ShareProfileButton({
  ...
  // Build a nice message for WhatsApp
  ...
  const whatsappMessage = `¡Hola! Échale un vistazo a mi perfil y garaje motero en QuedaMoto 🏍️💨:\n\n👤 *${username}*${city ? ` · 📍 ${city}` : ''}\n${vehiclesText ? `🏍️ Garaje: *${vehiclesText}*\n` : ''}\nMira mis motos y rutas aquí:\n${profileUrl}`
  ...
  function handleNativeShare(e: React.MouseEvent) {
    if (typeof navigator !== 'undefined' && navigator.share) {
      e.preventDefault()
      navigator.share({
        title: `Perfil de ${username} - QuedaMoto`,
        text: `Perfil de ${username} en QuedaMoto`,
        url: profileUrl,
      }).catch(() => {})
    }
  }
  ...
  return (
    <div className={`flex flex-col sm:flex-row items-stretch sm:items-center gap-2 ${className}`}>
      ...
      >
        ...
        <span>Compartir mi Garaje en WhatsApp</span>
      </a>
